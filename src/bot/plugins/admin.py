...
class Admin(Plugin):
    ...
    async def admin(self, message: Message, admin: AdminService = Provide[Container.admin_service]):
        admin_instance = Admin(username=message.sender_name, user_id=message.user_id)
        if await admin.check_if_admin(message.sender_name, message.user_id, admin_instance):
